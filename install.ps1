# Links the GoldGoal addon folder from this repo into the WoW retail AddOns
# folder as a directory junction, so edits here are live in the game after /reload.
# Run again to re-create the link. Remove with:  Remove-Item "<AddOns>\GoldGoal"
param(
    [string]$WowAddOns = "C:\Program Files (x86)\World of Warcraft\_retail_\Interface\AddOns"
)

$source = Join-Path $PSScriptRoot "GoldGoal"
$target = Join-Path $WowAddOns "GoldGoal"
